...
public class VoiceViewHolder<MESSAGE extends IMessage> extends BaseMessageViewHolder<MESSAGE>
      implements MessageListAdapter.DefaultMessageViewHolder, ViewHolderController.ReplayVoiceListener {
    ...
    public VoiceViewHolder(View itemView, boolean isSender) {
        ...
        if (!isSender) {
            ...
            mReceivePb = (ProgressBar) itemView.findViewById(R.id.pb_voice_loading);
        } else {
    ...
    public void onBind(final MESSAGE message) {
        ...
        if (mIsSender) {
            switch (message.getMessageStatus()) {
                case SEND_GOING:
                case CREATED:
                    if (mVoiceLoader != null) {
                        if (message.getDuration() != 0L) {
                            mLengthTv.setVisibility(View.VISIBLE);
                            mSendingPb.setVisibility(View.GONE);
                        } else {
                            mLengthTv.setVisibility(View.GONE);
                            mSendingPb.setVisibility(View.VISIBLE);
                        }
                        mVoiceLoader.loadVoice(message);
                    }
                    break;
                case SEND_FAILED:
                    mSendingPb.setVisibility(View.GONE);
            ...
            switch (message.getMessageStatus()) {
                ...
                case CREATED:
                    if (mVoiceLoader != null) {
                        if (message.getDuration() != 0L) {
                            mLengthTv.setVisibility(View.VISIBLE);
                            mReceivePb.setVisibility(View.GONE);
                        } else {
                            mLengthTv.setVisibility(View.GONE);
                            mReceivePb.setVisibility(View.VISIBLE);
                        }
                        mVoiceLoader.loadVoice(message);
                    }
                    break;
...
//                }
    ...
    public void playVoice(int position, MESSAGE message) {
        if (message.getDuration() == 0L) {
            ToastUtils.showShort("正在加载中，请稍候");
            return;
        }
