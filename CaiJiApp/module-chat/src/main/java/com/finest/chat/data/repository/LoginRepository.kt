package com.finest.chat.data.repository
 ...
 */
class LoginRepository @Inject constructor( @NetworkModule.WarnUrlRequest var request: HttpRequest){
...
//    }
   fun login(userName:String,psw:String): Flow<UIState<String>> = flow {
    ...
    fun testTextMsg(mes: MessageBean): Flow<UIState<String>> = flow {
        flow {
            val it = request.testFLOW(55)
            GetJSONHelper.checkCode(it)
            emit(it)
        }.catch {
            emit(UIState.error<String>(mes.msgId))
        }.collect {
            GetJSONHelper.checkCode(it)
            emit(UIState.success(mes.msgId))
        }

    }

    fun testImageMsg(mes: MessageBean): Flow<UIState<String>> = flow {
        flow {
            val it = request.testFLOW(55)
            GetJSONHelper.checkCode(it)
            emit(it)
